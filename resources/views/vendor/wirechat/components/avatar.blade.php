...
<div
    {{ $attributes->merge([
            'class' => "shrink-0 inline-flex items-center justify-center   relative transition
                      overflow-visible rounded-full border border-gray-200 text-gray-300 bg-gray-100
                      text-base ",
        ])->class(
            $story
    ...
    @if (!$src && $group)
        {{-- <svg class="shrink-0 scale-90 w-full h-full rounded-full text-gray-300 bg-gray-100" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 16 16">
          ...
          <svg class="shrink-0  scale-90 w-full h-full rounded-full text-gray-300 bg-gray-100"  xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true" data-slot="icon">
    ...
    @if ($disappearing)
    <span dusk="disappearing_messages_icon" class="absolute z-50 -bottom-1 bg-white -right-2 rounded-full p-px"> 
