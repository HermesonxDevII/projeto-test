...
<div id="new-chat-modal ">
    ...
    <div 
    class="relative w-full h-96  border mx-auto overflow-auto bg-white px-7 sm:max-w-lg sm:rounded-lg">
    ...
    <header class=" sticky top-0 bg-white z-10 py-2">
        <div class="flex justify-between items-center justify-between pb-2">
            ...
            <button
             dusk="close_modal_button"
                class="p-2  text-gray-600 rounded-full hover:text-gray-800 hover:bg-gray-50">
        ...
        <section class="flex flex-wrap items-center px-0 border-b">
            <input  dusk="search_users_field" autofocus type="search" id="users-search-field"
                ...
                class=" w-full border-0 w-auto px-0 outline-hidden focus:outline-hidden bg-none rounded-lg focus:ring-0 hover:ring-0">
    ...
    <div class="relative w-full">
        ...
        {{-- New Group button --}}
        @if (WireChat::showNewGroupModalButton() && getAuthenticatedParticipant()->canCreateGroups())

        {{-- Buton to trigger opening of new grop modal --}}
        <x-wirechat::actions.new-group widget="{{$this->isWidget()}}">
        <button  @dusk="open_new_group_modal_button"  class="flex items-center gap-3 my-4  rounded-lg p-2 w-full border hover:border-gray-300 transition-colors" >
            ...
            <p class="">@lang('wirechat::new.chat.actions.new_group.label')</p>
        </button>
                    ...
                    @foreach ($users as $key => $user)
                        <li wire:key="user-{{ $key }}"
                            ...
                            <p class="group-hover:underline transition-all">
                                {{ $user->name ? $user->name : $user->full_name }}</p>
