...
<footer class="shrink-0 h-auto relative   sticky bottom-0 mt-auto">
    ...
    @if ($conversation->isGroup() && !$conversation->group?->allowsMembersToSendMessages() && !$authParticipant->isAdmin())
        <div
            class="bg-gray-50 w-full text-center text-gray-600 justify-center text-sm flex py-4"
        > Apenas admins podem enviar mensagens </div>
    @else
        <div
            id="chat-footer"
            x-data="{ 'openEmojiPicker': false }"
            class=" px-3 md:px-1 border-t shadow-sm z-50  flex flex-col gap-3 items-center  w-full   mx-auto"
        >
            {{-- Emoji section , we put it seperate to avoid interfering as overlay for form when opened --}}
            <section
                wire:ignore
                x-cloak
                x-show="openEmojiPicker"
                ...
                class="w-full flex hidden sm:flex   py-2 sm:px-4 py-1.5 border-b  h-96 min-w-full"
            >
                <emoji-picker
                    dusk="emoji-picker"
                    style="width: 100%"
            ...
            <section
                class=" py-2 sm:px-4 py-1.5    z-50  bg-gray-50 flex flex-col gap-3 items-center  w-full mx-auto"
            >
                        ...
                        <div x-data="attachments('media')">
                            ...
                            >
                                ...
                                @foreach ($media as $key => $mediaItem)
                                    @if (str()->startsWith($mediaItem->getMimeType(), 'image/'))
                                        <div class="relative h-24 sm:h-36 aspect-4/3 ">
                                            ...
                                            <img
                                                class="h-full w-full  rounded-lg object-scale-down"
                                ...
                                <label
                                    ...
                                    class="shrink-0 cursor-pointer relative w-16 h-14 rounded-lg bg-gray-100 flex text-center justify-center border border-gray-50"
                                >
                                    <input
                                        wire:loading.attr="disabled"
                                        @change="handleFileSelect(event,{{ count($media) }})"
                                        type="file"
                                        multiple
                        ...
                        >
                            ...
                            @foreach ($files as $key => $file)
                                <div class="relative shrink-0">
                                    ...
                                    {{-- File details --}}
                                    <div
                                        class="flex items-center group overflow-hidden border rounded-xl">
                                        <span class=" p-2">
                                            {{-- document svg:HI --}}
                                            <svg
                                                xmlns="http://www.w3.org/2000/svg"
                            ...
                            <label
                                wire:loading.class="cursor-progress"
                                class="cursor-pointer shrink-0 relative w-16 h-14 rounded-lg bg-gray-100 transition-colors   flex text-center justify-center border border-gray-50"
                            >
                                <input
                                    wire:loading.attr="disabled"
                                    @change="handleFileSelect(event,{{ count($files) }})"
                                    type="file"
                                    multiple
                ...
                @if ($replyMessage != null)
                    <section class="p-px py-1 w-full col-span-12">
                        <div class="flex justify-between items-center">
                            <h6 class="text-sm">
                                    {{ $replyMessage?->ownedBy($this->auth)
                                        ? __('wirechat::chat.labels.replying_to_yourself')
                                        : __('wirechat::chat.labels.replying_to',['participant'=>$replyMessage->sendable?->name])
                                    }}
                            </h6>
                            <button
                                wire:loading.attr="disabled"
                                wire:click="removeReply()"
                ...
                >
                    ...
                    <div class="w-10 hidden sm:flex max-w-fit  items-center">
                        <button
                            ...
                            x-ref="emojibutton"
                            class=" disabled:cursor-progress rounded-full p-px"
                        >
                            <svg
                                x-bind:style="openEmojiPicker && { color: 'var(--wirechat-primary-color)' }"
                                viewBox="0 0 24 24"
                                height="24"
                                width="24"
                                preserveAspectRatio="xMidYMid meet"
                                class="w-7 h-7 text-gray-700 srtoke-[1.3]"
                                version="1.1"
                                x="0px"
                                y="0px"
                        ...
                        >
                            ...
                            >
                                <span dusk="upload-trigger-button">
                                    <svg
                                        class="size-6 w-7 h-7 text-gray-600"
                                        xmlns="http://www.w3.org/2000/svg"
                                        width="36"
                                        height="36"
                                        viewBox="0 0 24 24"
                            ...
                            <div class="grid gap-2 w-full ">
                                ...
                                @if (config('wirechat.allow_file_attachments', true))
                                    ...
                                    >
                                        ...
                                        <div
                                            class="w-full  flex items-center gap-3 px-1.5 py-2 rounded-md hover:bg-gray-100 cursor-pointer"
                                        >
                                            <span>
                                                <svg
                                                    xmlns="http://www.w3.org/2000/svg"
                                ...
                                @if (config('wirechat.allow_media_attachments', true))
                                    <label
                                        wire:loading.class="cursor-progress"
                                        x-data="attachments('media')"
                                        class="cursor-pointer"
                                    >
                                        {{-- Trigger image upload --}}
                                        <input
                                            dusk="media-upload-input"
                                            wire:loading.attr="disabled"
                                            wire:target="sendMessage"
                                            @change="handleFileSelect(event, {{ count($media) }})"
                                            type="file"
                    ...
                    <div @class(['flex gap-2 sm:px-2 w-full'])>
                        <textarea
                            ...
                            class="w-full disabled:cursor-progress resize-none h-auto max-h-20  sm:max-h-72 flex grow border-0 outline-0 focus:border-0 focus:ring-0  hover:ring-0 rounded-lg  focus:outline-hidden   "
                            x-init="
                                document.querySelector('emoji-picker').addEventListener('emoji-click', event => {
                                    const emoji = event.detail['unicode'];
                                    const inputField = $refs.body;
                    ...
                    >
                        ...
                        <button
                            ...
                            class=" ml-auto disabled:cursor-progress font-bold"
                        >
                            <svg
                                class="w-7 h-7"
                                xmlns="http://www.w3.org/2000/svg"
