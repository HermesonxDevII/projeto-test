...
<ul wire:loading.delay.long.remove wire:target="search" class="p-2 grid w-full spacey-y-2">
    ...
    if ($conversation->isPrivate() && !$conversation->isSelfConversation()) {
        ...
        $receiverParticipant = $conversation->participants
            ->first(fn($p) => $p->participantable_id !== $auth->id || $p->participantable_type !== get_class($auth));
    ...
    id="conversation-{{ $conversation->id }}" 
        ...
        wire:navigate href="{{ route(WireChat::viewRouteName(), $conversation->id) }}" @endif
            ...
            <aside class="grid  grid-cols-12 w-full">
                <div
                    class="col-span-10 border-b pb-2 border-gray-100 relative overflow-hidden truncate leading-5 w-full flex-nowrap p-1">
