...
<header
    class="px-3 z-10 sticky top-0 w-full py-2 "
    dusk="header"
>
    {{-- Title/name and Icon --}}
        ...
        <div class="flex gap-x-3 items-center  ">
            @if ($showNewChatModalButton)
                <x-wirechat::actions.new-chat widget="{{$this->isWidget()}}">
                    <button
                        id="open-new-chat-modal-button"
                        class=" flex items-center focus:outline-hidden"
                    >
                        <svg
                            class="w-8 h-8 -mb-1 text-gray-500 hover:text-gray-900"
            ...
            @if ($showHomeRouteButton)
                ...
                >                
                    <svg
                        class="bi bi-x-octagon-fill w-8 my-auto h-8 stroke-[0.9] text-gray-500 transition-colors duration-300 hover:text-gray-900"
                        xmlns="http://www.w3.org/2000/svg"
                        width="32"
                        height="32"
                        viewBox="0 0 24 24"
