...
class Chats extends Component
{
    ...
    public function getListeners()
    {
        ...
        return [
            ...
            "echo-private:participant.{$encodedType}.{$userId},.App\\Events\\NotifyParticipant" => 'refreshComponent',
        ];
    }
    ...
    protected function loadConversations()
    {
        ...
        $additionalConversations = $this->auth->conversations()
            ->with([
                ...
                'lastMessage.sendable', // pega as ultimas conversas pelo created_at
                'authParticipant' => fn ($query) => $query->select('id', 'participantable_id', 'participantable_type', 'conversation_id', 'conversation_read_at'), // Pega as conversas onde o participant logado seja o dono da conversa
            ...
            ->when(trim($this->search ?? '') == '', fn ($query) => $query->withoutDeleted()) // Exclude blanks & deleted. /* Antes tinha o ->withoutBlanks() depois do withtouDeleted */
            ->latest('updated_at')
            ->skip($offset)
            ->take($perPage)
