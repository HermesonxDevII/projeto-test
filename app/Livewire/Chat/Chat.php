...
use App\Jobs\NotifyParticipants;
...
class Chat extends Component
{
    ...
    public function getListeners()
    {
        ...
        return [
            'refresh' => '$refresh',
            'echo-private:conversation.'.$conversationId.',.App\\Events\\MessageCreated' => 'appendNewMessage',
            'echo-private:conversation.'.$conversationId.',.App\\Events\\MessageDeleted' => 'removeDeletedMessage',
    ...
    public function removeDeletedMessage($event)
    {
        ...
        if ($event['message']['conversation_id'] == $this->conversation->id) {
            ...
            // Make sure message does not belong to auth
            if ($event['message']['sendable_id'] == $this->auth()->id && $event['message']['sendable_type'] === $this->auth->getMorphClass()) {
                return null;
            }
    ...
    public function appendNewMessage($event)
    {
        ...
        if ($event['message']['conversation_id'] == $this->conversation->id) {
            ...
            // Make sure message does not belong to auth
            if ($newMessage->sendable_id == $this->auth()->id && $newMessage->sendable_type == $this->auth->getMorphClass()) {
                return null;
            }
    ...
    protected function rateLimit()
    {
        ...
        if (RateLimiter::tooManyAttempts('send-message:'.$this->auth()->id, $perMinute)) {

            return abort(429, __('wirechat::chat.messages.rate_limit'));
        }

        RateLimiter::increment('send-message:'.$this->auth()->id);
    }
    ...
    {
        abort_unless($this->auth(), 401); // aqui
    ...
    {
        return getAuthenticatedParticipant();
    }
    ...
    private function initializeParticipants()
    {
        if (in_array($this->conversation->type, [ConversationType::PRIVATE, ConversationType::SELF])) {
            ...
            $this->authParticipant = $participants->first(function ($p) {
                return $p->participantable_id === $this->auth()->id &&
                    $p->participantable_type === get_class($this->auth());
            });

            $this->receiverParticipant = $participants->first(function ($p) {
                return $p->participantable_id !== $this->auth()->id ||
                    $p->participantable_type !== get_class($this->auth());
            });
