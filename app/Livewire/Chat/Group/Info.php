...
class Info extends ModalComponent
{
    ...
    public function deleteGroup()
    {
        ...
        abort_unless(getAuthenticatedParticipant()->belongsToConversation($this->conversation), 403, 'Forbidden: You do not have permission to delete this group.');

        abort_if($this->conversation->isPrivate(), 403, 'Operation not allowed: Private chats cannot be deleted.');

        abort_unless(getAuthenticatedParticipant()->isOwnerOf($this->conversation), 403, 'Forbidden: You do not have permission to delete this group.');

        // Ensure all participants are removed before deleting the group
        $participantCount = $this->conversation->participants()
            ->withoutParticipantable(getAuthenticatedParticipant())
    ...
    public function exitConversation()
    {
        ...
        $auth = getAuthenticatedParticipant();
